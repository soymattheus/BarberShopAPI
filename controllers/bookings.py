from models.bookings import get_bookins_model, update_booking_model, create_booking_model
...
def get_bookings_controller(current_user, id_user):
    ...
    try:
        rows = get_bookins_model(id_user)
...
def create_booking_controller(current_user, id_user, date, time, service_id, barber_id):
    try:
        if id_user != current_user['user_id']:
            return {'error': 'Acesso não autorizado'}, 403
